/*
 * This module file defines the methods of the gpsInput_t class for use in
 * reading data from a GPS receiver
 */
...
gpsInput_t::gpsInput_t()
	: dev(0)
	, parsing("")
{
	char *descr=getenv("GPSDEV");
	if (descr) {
		descr = strdup(descr);
...
void gpsInput_t::readData(int fd)
{
	char buffer[32];
	int n ;
