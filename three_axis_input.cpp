/*
 * This module file defines the methods of the three_axis_input_t 
 * class for use in reading Linux input events
 */
...
three_axis_input_t::three_axis_input_t(char const *labelfrag)
	: dev(0)
	, xmax(0)
	, ymax(0)
	, zmax(0)
...
three_axis_input_t::~three_axis_input_t()
{
}

void three_axis_input_t::readData(int fd)
{
	...
	while (0 <= (n=::read(fd,buffer,sizeof(buffer)))) {
		...
		for (int i = 0 ; i < n ; i++) {
			...
			if (EV_ABS == event.type) {
				...
				emit axis_change(1.0*x,1.0*y,1.0*z);
			}
		}
	}
}
